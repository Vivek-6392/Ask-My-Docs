...
#     """Replace [1], [2] citations with stylized inline pill badges."""
#     escaped = html.escape(text)

#     def _replace_tag(match: re.Match) -> str:
#         idx = match.group(1)
#         return f'<span class="citation-pill">{idx}</span>'

#     formatted = _CITATION_REF_RE.sub(_replace_tag, escaped)
...
#     # Retrieval Settings
#     st.markdown(
#         '<div class="sidebar-section-title"><span>Retrieval settings</span><span>⚙</span></div>',
#         unsafe_allow_html=True,
#     )
...
#             formatted_html = _format_inline_citations(clean_content)
#             st.markdown(
#                 f"""
...
#         formatted_html = _format_inline_citations(clean_answer)
...
#     count = len(citations)
#     st.markdown(
#         f'<div class="sources-header">🗎 {count} source{"s" if count != 1 else ""}</div>',
#         unsafe_allow_html=True,
#     )
